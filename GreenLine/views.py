...
def main(request) :
    ...
    if (request.method != 'POST') :
        ...
        town_code = request.POST['town']
        consignee_id = int(request.POST['consignee'])
        print(prefecture_id, city_code, town_code, consignee_id)
        ...
        if request.POST["radio"] == "name" :
            ...
            if consignee_id and consignee_id > 0 :
                consignees = consignees.filter(id=consignee_id).all()
            elif town_code and len(town_code) > 0 :
                consignees = consignees.filter(town=town_code).all()
            elif city_code and len(city_code) > 0 :
                towns = Town.objects.filter(city=city_code).all()
                consignees = consignees.filter(town__in=towns).all()
            elif prefecture_id and prefecture_id > 0 :
                cities = City.objects.filter(prefecture=prefecture_id).all()
                towns = Town.objects.filter(city__in=cities).all()
                consignees = consignees.filter(town__in=towns).all()
        ...
        params['city_selected'] = city_code
        params['town_list'] = get_town_select_from_shipper(shipper_id, city_code)
        params['town_selected'] = town_code
        params['consignee_list'] = get_consignee_select_from_shipper(shipper, prefecture_id, city_code, town_code)
...
def consignee(request, id, edit) :
    ...
    form.fields['zip'].initial = zip
    form.fields['prefecture'].initial = target.town.city.prefecture.id
    form.fields['city'].initial = target.town.city.code
    form.fields['town'].initial = target.town.code
    params = {
        'title' : 'Consignee',
        'msg' : '',
        'name' : employee.name,
        ...
        'form' : form,
        'model' : target,
        'town_selected' : target.town.code,
        'town_list' : get_town_select_from_zip(zip),
        'city_selected' : target.town.city.code,
        'city_list' : get_city_select_from_zip(zip)
    }
    if edit == 2 :
        params['msg'] = '本当に削除しますか？'
    elif edit == 4 :
        ...
        if request.POST['zip'] :
            ...
            form.fields['prefecture'].initial = Town.objects.filter(zip=zip).first().city.prefecture.id
            params['city_list'] = get_city_select_from_zip(zip)
            form.fields['city'].initial = Town.objects.filter(zip=zip).first().city.code
            params['city_selected'] = Town.objects.filter(zip=zip).first().city.code
            params['town_list'] = get_town_select_from_zip(zip)
        elif request.POST['prefecture'] :
            ...
            if request.POST["city"] :
                ...
                params['city_selected'] = request.POST["city"]
                params['town_list'] = get_town_select(request.POST['city'])
        if request.POST["town"] :
            form.fields['town'].initial = request.POST["town"]
            params['town_selected'] = int(request.POST["town"])
        if form.is_valid() :
            if edit == 5 :
                params['msg'] = 'この内容で更新してよいですか？'
            else :
                form.instance.id = id
                form.instance.town = Town.objects.filter(code=form.cleaned_data['town']).first()
                form.save()
                return redirect('show_consignee')
        else :
            errors = form.errors.as_data()
...
def add_consignee(request) :
    ...
    if request.POST :
        ...
        if request.POST["city"] :
            ...
            params['town_list'] = get_town_select(request.POST['city'])
        if request.POST["town"] :
            params['town_selected'] = request.POST["town"]
        if request.POST['zip'] :
            zip = request.POST['zip']
            params['city_list'] = get_city_select_from_zip(zip)
            params['town_list'] = get_town_select_from_zip(zip)
...
def get_city_select(prefecture_id) :
    ...
    city_list = []
    for city in cities:
        city_list.append({'code': city.code, 'name': city.name})
    return city_list

def get_city_select_from_zip(zip) :
    towns = Town.objects.filter(zip=zip).order_by('id')
    cities = City.objects.filter(code__in=towns.values_list('city', flat=True))
    city_list = []
    for city in cities:
        city_list.append({'code': city.code, 'name': city.name})
    return city_list
...
def get_towns(request) :
    ...
    return JsonResponse({'towns': get_town_select(city_code)})

def get_town_select(city_code) :
    city = City.objects.filter(code=city_code).first()
    towns = Town.objects.filter(city=city).order_by('id')
    town_list = []
    for town in towns:
        town_list.append({'code': town.code, 'name': town.name})
    return town_list

def get_town_select_from_zip(zip) :
    towns = Town.objects.filter(zip=zip).order_by('id')
    town_list = []
    for town in towns:
        town_list.append({'code': town.code, 'name': town.name})
...
def get_address_from_zip(request) :
    ...
    city = {'code': 0, 'name': ''}
    prefecture = {'id': 0, 'name': ''}
    town_list = get_town_select_from_zip(zip)
    if len(town_list) > 0 :
        town = Town.objects.filter(zip=zip).order_by('id').first()
        city = {'code': town.city.code, 'name': town.city.name}
        prefecture = {'id': town.city.prefecture.id, 'name': town.city.prefecture.name}
    print(town_list)
    print(city)
    print(prefecture)
    return JsonResponse({'towns': town_list, 'city': city, 'prefecture': prefecture})

@csrf_exempt
def get_consignees(request) :
    town_code = request.GET.get('town_code')
    return JsonResponse({'consignees': get_consignee_select(town_code)})

def get_consignee_select(town_code) :
    town = Town.objects.filter(code=town_code).first()
    consignees = Consignee.objects.filter(town=town).order_by('id')
    consignee_list = []
...
def get_zip(request) :
    town_code = request.GET.get('town_code')
    zip = Town.objects.filter(code=town_code).first().zip
...
def get_prefecture_select_from_shipper(shipper_id) :
    ...
    towns = Town.objects.filter(code__in=consignees.values_list('town', flat=True))
    cities = City.objects.filter(code__in=towns.values_list('city', flat=True))
...
def get_city_select_from_shipper(shipper_id, prefecture_id) :
    consignees = Consignee.objects.filter(shipper=shipper_id).order_by('id')
    towns = Town.objects.filter(code__in=consignees.values_list('town', flat=True))
    cities = City.objects.filter(code__in=towns.values_list('city', flat=True), prefecture_id=prefecture_id).order_by('id')
    city_list = [{'code': '', 'name': '---------'}]
    for city in cities :
        city_list.append({'code': city.code, 'name': city.name})
    print(city_list)
...
@csrf_exempt
def get_towns_from_shipper(request) :
    shipper_id = request.GET.get('shipper_id')
    city_code = request.GET.get('city_code')
    town_list = get_town_select_from_shipper(shipper_id, city_code)
    print(town_list)
    return JsonResponse({'towns': town_list})

def get_town_select_from_shipper(shipper_id, city_code) :
    consignees = Consignee.objects.filter(shipper=shipper_id).order_by('id')
    city = City.objects.filter(code=city_code).first()
    towns = Town.objects.filter(code__in=consignees.values_list('town', flat=True), city=city)
    town_list = [{'code': '', 'name': '---------'}]
    for town in towns :
        town_list.append({'code': town.code, 'name': town.name})
    print(town_list)
...
def get_consignees_from_shipper(request) :
    ...
    city_code = request.GET.get('city_code')
    town_code = request.GET.get('town_code')
    print(shipper_id, prefecture_id, city_code, town_code)
    consignee_list = get_consignee_select_from_shipper(shipper_id, prefecture_id, city_code, town_code)
    return JsonResponse({'consignees': consignee_list})

def get_consignee_select_from_shipper(shipper, prefecture_id, city_code, town_code) :
    print(type(prefecture_id), prefecture_id)
    consignees = Consignee.objects.filter(shipper=shipper).order_by('id')
    if town_code and len(town_code) > 0 :
        towns = Town.objects.filter(code=town_code).all()
        print("towns", towns)
        consignees = consignees.filter(town__in=towns).all()
    elif city_code and len(city_code) > 0 :
        cities = City.objects.filter(code=city_code).all()
        print("cities", cities)
        towns = Town.objects.filter(city__in=cities).all()
        consignees = consignees.filter(town__in=towns).all()
    elif prefecture_id :
        prefecture = Prefecture.objects.filter(id=int(prefecture_id)).first()
        print("prefecture", prefecture)
        cities = City.objects.filter(prefecture=prefecture)
        towns = Town.objects.filter(city__in=cities).all()
        consignees = consignees.filter(town__in=towns).all()
